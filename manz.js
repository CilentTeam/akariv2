...
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, reSize, await, sleep } = require('./lib/myfunc')
...
async function startalfa() {
    const alfa = alfaConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Alfa Full Bug','firefox','1.0.0'],
        auth: state
    })
    ...
    console.log(color(figlet.textSync(`Cilent-MD`, {
font: 'Standard',
...
whitespaceBreak: true
    ...
    alfa.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./main")(alfa, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
	...
	/* alfa.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            let metadata = await alfa.groupMetadata(anu.id)
            let participants = anu.participants
            for (let num of participants) {
                // Get Profile Picture User
                try {
                    ppuser = await alfa.profilePictureUrl(num, 'image')
                } catch {
                ...
                try {
                    ppgroup = await alfa.profilePictureUrl(anu.id, 'image')
                } catch {
                    ppgroup = 'https://tinyurl.com/yx93l6da'
                }

                if (anu.action == 'add') {
                    alfa.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Welcome To ${metadata.subject} @${num.split("@")[0]}\nDeskripsi:\n${metadata.desc}\n\nPenting Jika Kalian Out Dari Gc Langsung Di Send Bug` })
                } else if (anu.action == 'remove') {
                    alfa.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
                    alfa.sendMessage(num, { image: { url: ppuser }, mentions: [num], caption: `Kenapa Out Bang ?` })
                } else if (anu.action == 'promote') {
                    alfa.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Promote From ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    alfa.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Demote From ${metadata.subject}` })
              }
            }
        } catch (err) {
            console.log(err)
	...
	for (let i of kon) {
	    list.push({
	    	displayName: `${copy}`,
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${copy}\nFN:${copy}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:manz213@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://manz-api.herokuapp.com\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	alfa.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
    }
    ...
    alfa.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); alfa.logout(); }
        ...
        if (connection === 'open') {
        alfa.sendText("6281327496283@s.whatsapp.net", `Halo Creator Ijin Pake Sc Bot Cilent-MD V1. Makasihh`)
        }
    ...
    // Add Other
      ...
      * @param {*} options
      */
     alfa.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
      let mime = '';
      let res = await axios.head(url)
      mime = res.headers['content-type']
      if (mime.split("/")[1] === "gif") {
     return alfa.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback: true, ...options}, { quoted: quoted, ...options})
      }
      let type = mime.split("/")[0]+"Message"
      if(mime === "application/pdf"){
     return alfa.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "image"){
     return alfa.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options}, { quoted: quoted, ...options})
      }
      if(mime.split("/")[0] === "video"){
     return alfa.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "audio"){
     return alfa.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options}, { quoted: quoted, ...options })
      }
      }
    ...
    /** Send List Messaage
      ...
      *@param {*} quoted
      */
        alfa.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
        let sections = sects
        var listMes = {
        text: text,
        footer: footer,
        ...
        alfa.sendMessage(jid, listMes, { quoted: quoted })
        }
     ...
     * @returns 
     */
        alfa.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
        ...
        alfa.sendMessage(jid, templateMessage)
        }
...
**/
        ...
        catalog
**/
alfa.sendOrder = async(jid, text, orid, img, itcount, title, sellers, tokens, ammount) => {
const order = generateWAMessageFromContent(jid, proto.Message.fromObject({
"orderMessage": {
"orderId": 9999999, 
"thumbnail": fs.readFileSync(`./lib/thx.jpg`),
"itemCount": 999999, 
"status": "INQUIRY", 
"surface": "CATALOG", 
"orderTitle": "MANZ IS HARD",
"message": `${text}`, 
...
alfa.relayMessage(jid, order.message, { messageId: order.key.id})
        ...
        send but loc
**/
alfa.send5Loc = async (jid , text = '' , footer = '', img = 'fs.readFileSync(`./lib/test.jpg`)', but = [], options = {}) =>{
     ...
     * @returns
     */
    alfa.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: vid }, { upload: alfa.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
     ...
     * @returns
     */
    alfa.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: alfa.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
        videoMessage: message.videoMessage,
